...
function normalizeOptions (options) {
  ...
  const etag = options.etag !== undefined
    ? options.etag === 'strong' ? 'strong' : false
    : true
...
function sendFileDirectly (request, path, stat, options) {
  ...
  if (options.etag) {
    console.log('options.etag', options.etag)
    const hdr = options.etag === 'strong' ? '"' : 'W/"'
    const etag = hdr + stat.size.toString(16) + '-' + stat.mtime.getTime().toString(16) + '"'
    debug('etag %s', etag)
    headers.ETag = etag
  }
